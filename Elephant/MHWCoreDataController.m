...
- (BOOL)isMigrationNeededOfStoreURL:(NSURL*) storeURL ofType:(NSString*)storeType forNewModel:(NSManagedObjectModel*) newModel {
    ...
    NSError* error;
    NSDictionary *sourceMetadata = [NSPersistentStoreCoordinator metadataForPersistentStoreOfType:storeType URL: storeURL error: &error];
    
    if (sourceMetadata != nil) {
        needMigration = ![newModel isConfiguration:nil compatibleWithStoreMetadata:sourceMetadata];
        NSLog(@"needMigration:%d", needMigration);
    } else {
        NSLog(@"with error code:%ld error message:%@", (long)[error code], [error description]);
    }
...
- (BOOL)migrateOfStoreURL:(NSURL*) storeURL ofType:(NSString*)storeType forNewModel:(NSManagedObjectModel*) newModel ifError: (NSError *__autoreleasing *)error {
    ...
    BOOL OK = [migrationManager progressivelyMigrateURL: storeURL
                                                 ofType: storeType
                                                toModel: newModel
                                                  error:error];
    if (OK) {
        ...
        NSLog(@"migration failed with error code:%ld and error message:%@",  [*error code], [*error description] );
    }
...
{
    NSLog(@"migration manager mapping models for source mode ");
